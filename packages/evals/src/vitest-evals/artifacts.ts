...
export const PI_SESSION_SNAPSHOT_ARTIFACT = "tangentSessionJsonl";
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@tangent-ai/tangent-evals:session" &&
				artifact.type !== "@tangent-ai/tangent-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@tangent-ai/tangent-evals:session" ? "sessions" : "sources";
