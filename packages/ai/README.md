...
```bash
npm install @tangent-ai/tangent-ai
```

TypeBox exports are re-exported from `@tangent-ai/tangent-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@tangent-ai/tangent-ai';
...
import { openaiProvider } from '@tangent-ai/tangent-ai/providers/openai';
import { openrouterProvider } from '@tangent-ai/tangent-ai/providers/openrouter';
import { amazonBedrockProvider } from '@tangent-ai/tangent-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@tangent-ai/tangent-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@tangent-ai/tangent-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@tangent-ai/tangent-ai';
...
API-key credentials use the same discriminator as tangent's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@tangent-ai/tangent-ai';
...
```typescript
import { validateToolCall, type Tool } from '@tangent-ai/tangent-ai';
...
```typescript
import { builtinImagesModels } from '@tangent-ai/tangent-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@tangent-ai/tangent-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@tangent-ai/tangent-ai';
import { openAICompletionsApi } from '@tangent-ai/tangent-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@tangent-ai/tangent-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@tangent-ai/tangent-ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are tangent thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@tangent-ai/tangent-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@tangent-ai/tangent-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@tangent-ai/tangent-ai';
import { anthropicProvider } from '@tangent-ai/tangent-ai/providers/anthropic';
import { openaiProvider } from '@tangent-ai/tangent-ai/providers/openai';
import { googleProvider } from '@tangent-ai/tangent-ai/providers/google';
...
- `@tangent-ai/tangent-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@tangent-ai/tangent-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@tangent-ai/tangent-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@tangent-ai/tangent-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@tangent-ai/tangent-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@tangent-ai/tangent-ai/api/bedrock-converse-stream.lazy';
...
```bash
npx @tangent-ai/tangent-ai login              # interactive provider selection
npx @tangent-ai/tangent-ai login anthropic    # login to specific provider
npx @tangent-ai/tangent-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@tangent-ai/tangent-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@tangent-ai/tangent-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@tangent-ai/tangent-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@tangent-ai/tangent-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@tangent-ai/tangent-ai`.
