...
import type { Component, EditorTheme, TUI } from "@tangent-ai/tangent-tui";
import { truncateToWidth, visibleWidth } from "@tangent-ai/tangent-tui";
...
export default function (tangent: ExtensionAPI) {
	...
	tangent.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await tangent.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = tangent.getThinkingLevel();
