...
tangent --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.tangent/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `tangent.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs tangent theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `tangent.events` |
