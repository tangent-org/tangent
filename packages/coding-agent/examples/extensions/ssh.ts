 ...
 *
 * Usage:
 *   tangent -e ./ssh.ts --ssh user@host
 *   tangent -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (tangent: ExtensionAPI) {
	tangent.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	tangent.on("session_start", async (_event, ctx) => {
		...
		const arg = tangent.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
