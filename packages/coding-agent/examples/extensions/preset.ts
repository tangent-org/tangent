 ...
 * - ~/.tangent/agent/presets.json (global)
 ...
 *
 * Usage:
 * - `tangent --preset plan` - start with plan preset
...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@tangent-ai/tangent-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@tangent-ai/tangent-tui";
...
export default function presetExtension(tangent: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: tangent.getThinkingLevel(),
				tools: tangent.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			tangent.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = tangent.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				tangent.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await tangent.setModel(originalState.model);
				}
				tangent.setThinkingLevel(originalState.thinkingLevel);
				tangent.setActiveTools(originalState.tools);
			} else {
				tangent.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	tangent.registerShortcut(Key.ctrlShift("u"), {
	...
	tangent.registerCommand("preset", {
	...
	tangent.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = tangent.getFlag("preset");
	...
	tangent.on("turn_start", async () => {
		if (activePresetName) {
			tangent.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
