...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@tangent-ai/tangent-coding-agent";

export default function (tangent: ExtensionAPI) {
	tangent.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
