...
export default function (tangent: ExtensionAPI) {
	...
	tangent.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "tangent RPC Demo (new session)" : "tangent RPC Demo");
	...
	tangent.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	tangent.on("turn_end", async (_event, ctx) => {
	...
	tangent.registerCommand("rpc-input", {
	...
	tangent.registerCommand("rpc-editor", {
	...
	tangent.registerCommand("rpc-prefill", {
