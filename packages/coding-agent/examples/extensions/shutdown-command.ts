 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit tangent cleanly.
 */
...
export default function (tangent: ExtensionAPI) {
	// Register a /quit command that cleanly exits tangent
	tangent.registerCommand("quit", {
		description: "Exit tangent cleanly",
	...
	tangent.registerTool({
		...
		description: "Complete a task and exit tangent",
	...
	tangent.registerTool({
		...
		description: "Deploy the application and exit tangent",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await tangent.exec("npm", ["run", "deploy", params.environment], { signal });
