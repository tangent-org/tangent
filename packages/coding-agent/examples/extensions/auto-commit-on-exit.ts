...
export default function (tangent: ExtensionAPI) {
	tangent.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await tangent.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[tangent] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await tangent.exec("git", ["add", "-A"]);
		const { code: commitCode } = await tangent.exec("git", ["commit", "-m", commitMessage]);
