...
async function checkDirtyRepo(
	...
	const { stdout, code } = await tangent.exec("git", ["status", "--porcelain"]);
...
export default function (tangent: ExtensionAPI) {
	tangent.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(tangent, ctx, action);
	});

	tangent.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(tangent, ctx, "fork");
	});
}
