 ...
 *
 * Start tangent with this extension:
 *   tangent -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(tangent: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await tangent.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (tangent: ExtensionAPI) {
	tangent.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await tangent.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await tangent.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await tangent.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await tangent.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await tangent.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await tangent.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(tangent, ctx.cwd);
		if (conflicts.length === 0) return;

		tangent.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}
