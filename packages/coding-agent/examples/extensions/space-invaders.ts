...
import { isKeyRelease, Key, matchesKey, visibleWidth } from "@tangent-ai/tangent-tui";
...
export default function (tangent: ExtensionAPI) {
	tangent.registerCommand("invaders", {
		...
		handler: async (_args, ctx) => {
			...
			await ctx.ui.custom((tui, _theme, _kb, done) => {
				return new SpaceInvadersComponent(
					...
					() => done(undefined),
					(state) => {
						tangent.appendEntry(INVADERS_SAVE_TYPE, state);
					},
					savedState,
				);
			});
