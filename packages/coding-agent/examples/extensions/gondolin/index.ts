 ...
 *
 * Runs tangent's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   tangent -e /path/to/tangent/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (tangent: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `tangent ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	tangent.registerCommand("gondolin", {
	...
	tangent.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
