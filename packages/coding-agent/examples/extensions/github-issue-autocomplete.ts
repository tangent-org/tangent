...
async function resolveGitHubRepo(tangent: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await tangent.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (tangent: ExtensionAPI): void {
	tangent.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(tangent, ctx.cwd);
		if (!resolvedRepo.ok) {
		...
		const getIssues = async (): Promise<GitHubIssue[] | undefined> => {
			issuesPromise ||= (async () => {
				const result = await tangent.exec(
					"gh",
					[
						"issue",
						"list",
