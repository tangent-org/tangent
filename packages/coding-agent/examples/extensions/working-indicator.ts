 ...
 * working indicator shown while tangent is streaming a response.
 *
 * Usage:
 *   tangent --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore tangent's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "@tangent-ai/tangent-coding-agent";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "tangent default spinner";
	}
}
...
export default function (tangent: ExtensionAPI) {
	...
	tangent.registerCommand("working-indicator", {
