 ...
 *
 *   mkdir -p ~/.tangent/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.tangent/agent/extensions/
 *
 * Or:
 *
 *   tangent -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .pi, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@tangent-ai/tangent-coding-agent";
...
export default function (tangent: ExtensionAPI) {
	...
	tangent.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
