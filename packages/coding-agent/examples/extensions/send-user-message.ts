 ...
 *
 * Demonstrates tangent.sendUserMessage() for sending user messages from extensions.
 * Unlike tangent.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (tangent: ExtensionAPI) {
	...
	tangent.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			tangent.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	tangent.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				tangent.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	tangent.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				tangent.sendUserMessage(args, { deliverAs: "followUp" });
	...
	tangent.registerCommand("askwith", {
