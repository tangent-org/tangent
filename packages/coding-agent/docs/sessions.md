...
Sessions auto-save to `~/.tangent/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.
...
tangent --session <path|id> # Use a specific session file or partial session ID
tangent --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `tangent -r` opens the same picker at startup.
...
When available, tangent uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
tangent --name "Refactor auth module"
tangent --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `tangent -r`.
...
When `/tree` switches away from one branch to another, tangent can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
