...
**Source files** ([tangent](https://github.com/earendil-works/tangent)):
- [`packages/coding-agent/src/core/compaction/compaction.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/compaction.ts) - Auto-compaction logic
- [`packages/coding-agent/src/core/compaction/branch-summarization.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/branch-summarization.ts) - Branch summarization
- [`packages/coding-agent/src/core/compaction/utils.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/utils.ts) - Shared utilities (file tracking, serialization)
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/session-manager.ts) - Entry types (`CompactionEntry`, `BranchSummaryEntry`)
- [`packages/coding-agent/src/core/extensions/types.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/extensions/types.ts) - Extension event types

For TypeScript definitions in your project, inspect `node_modules/@tangent-ai/tangent-coding-agent/dist/`.
...
By default, `reserveTokens` is 16384 tokens (configurable in `~/.tangent/agent/settings.json` or `<project-dir>/.tangent/settings.json`). This leaves room for the LLM's response.
...
1. **Find cut point**: Walk backwards from newest message, accumulating token estimates until `keepRecentTokens` (default 20k, configurable in `~/.tangent/agent/settings.json` or `<project-dir>/.tangent/settings.json`) is reached
...
Defined in [`session-manager.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/session-manager.ts):
...
See [`prepareCompaction()`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/compaction.ts) and [`compact()`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/compaction.ts) for the implementation. For direct programmatic summarization, `generateSummary()` returns the summary text and `generateSummaryWithUsage()` returns `{ text, usage }`.
...
Both compaction and branch summarization track files cumulatively. When generating a summary, tangent extracts file operations from:
...
See [`collectEntriesForBranchSummary()`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/branch-summarization.ts), [`prepareBranchEntries()`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/branch-summarization.ts), and [`generateBranchSummary()`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/branch-summarization.ts) for the implementation.
...
Before summarization, messages are serialized to text via [`serializeConversation()`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/compaction/utils.ts):
...
Extensions can intercept and customize both compaction and branch summarization. See [`extensions/types.ts`](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/extensions/types.ts) for event type definitions.
...
```typescript
import { convertToLlm, serializeConversation } from "@tangent-ai/tangent-coding-agent";
...
Configure compaction in `~/.tangent/agent/settings.json` or `<project-dir>/.tangent/settings.json`:
