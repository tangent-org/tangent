...
| `.tangent/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`TANGENT_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so tangent resumes after the editor exits:
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.tangent/agent/npm/`; project-scoped npm packages install under `.pi/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
Paths in `~/.tangent/agent/settings.json` resolve relative to `~/.tangent/agent`. Paths in `.tangent/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
...
Project settings (`.tangent/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.tangent/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .tangent/settings.json (project)
{
