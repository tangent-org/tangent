> tangent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to tangent's agent capabilities. Use it to embed tangent in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @tangent-ai/tangent-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@tangent-ai/tangent-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `tangent.sendMessage()`.
...
The `Agent` class (from `@tangent-ai/tangent-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.tangent/agent", // default (expands ~)
});
```
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.tangent/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@tangent-ai/tangent-ai";
import { ModelRuntime } from "@tangent-ai/tangent-coding-agent";

const modelRuntime = await ModelRuntime.create();

// create() restores cached catalogs but does not refresh them from tangent.dev by default.
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.tangent/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `PI_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "@tangent-ai/tangent-ai";
import { createAgentSession, ModelRuntime } from "@tangent-ai/tangent-coding-agent";

// Default: uses ~/.tangent/agent/auth.json and ~/.tangent/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@tangent-ai/tangent-coding-agent";
...
```typescript
import { createAgentSession } from "@tangent-ai/tangent-coding-agent";
...
import { createAgentSession, defineTool } from "@tangent-ai/tangent-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `tangent.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `tangent.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.tangent/agent/extensions/`, `.pi/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@tangent-ai/tangent-coding-agent";
...
**Event Bus:** Extensions can communicate via `tangent.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@tangent-ai/tangent-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@tangent-ai/tangent-coding-agent";
...
1. Global: `~/.tangent/agent/settings.json`
2. Project: `<cwd>/.tangent/settings.json`
