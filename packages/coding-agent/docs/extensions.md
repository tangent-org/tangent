> tangent can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend tangent's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.tangent/agent/extensions/` (global) or `.pi/extensions/` (project-local) for auto-discovery. Use `tangent -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `tangent.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `tangent.registerCommand()`
- **Session persistence** - Store state that survives restarts via `tangent.appendEntry()`
...
Create `~/.tangent/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.tangent/agent/extensions/*.ts` | Global (all projects) |
| `~/.tangent/agent/extensions/*/index.ts` | Global (subdirectory) |
...
To share extensions via npm or git as tangent packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@tangent-ai/tangent-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@tangent-ai/tangent-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@tangent-ai/tangent-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed tangent packages installed with `tangent install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (tangent: ExtensionAPI) {
  // Subscribe to events
  tangent.on("event_name", async (event, ctx) => {
...
If the factory returns a `Promise`, tangent awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `tangent.registerProvider()` are flushed.
...
export default async function (tangent: ExtensionAPI) {
  ...
  tangent.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `tangent --list-models`.
...
/name or tangent.setSessionName()
...
thinking level changes (settings, keybinding, tangent.setThinkingLevel())
...
Fired before tangent decides whether to trust a project with dynamic configs (`.pi` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
tangent.on("project_trust", async (event, ctx) => {
...
```typescript
tangent.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
Fired when the current session display name is set via `/name`, RPC, or `tangent.setSessionName()`.

```typescript
tangent.on("session_info_changed", async (event, ctx) => {
...
```typescript
tangent.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, tangent emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
tangent.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, tangent emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
```typescript
tangent.on("session_before_compact", async (event, ctx) => {
...
tangent.on("session_compact", async (event, ctx) => {
...
tangent.on("session_compact_failed", async (event, ctx) => {
  // event.reason - "manual" (/compact), "threshold", or "overflow"
...
```typescript
tangent.on("session_before_tree", async (event, ctx) => {
...
tangent.on("session_tree", async (event, ctx) => {
...
```typescript
tangent.on("session_shutdown", async (event, ctx) => {
...
```typescript
tangent.on("before_agent_start", async (event, ctx) => {
...
tangent.on("agent_end", async (event, ctx) => {
...
```typescript
tangent.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

tangent.on("turn_end", async (event, ctx) => {
...
```typescript
tangent.on("message_start", async (event, ctx) => {
  // event.message
});

tangent.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

tangent.on("message_end", async (event, ctx) => {
...
```typescript
tangent.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

tangent.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

tangent.on("tool_execution_end", async (event, ctx) => {
...
```typescript
tangent.on("context", async (event, ctx) => {
...
```typescript
tangent.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header tangent adds for this call
...
```typescript
tangent.on("model_select", async (event, ctx) => {
...
```typescript
tangent.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `tangent.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, tangent waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@tangent-ai/tangent-coding-agent";
...
```typescript
import { isBashToolResult } from "@tangent-ai/tangent-coding-agent";

tangent.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@tangent-ai/tangent-coding-agent";

tangent.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap tangent's built-in local bash backend
...
```typescript
tangent.on("input", async (event, ctx) => {
...
Use `ctx.modelRegistry.streamSimple(model, context, options)` for provider-neutral options such as `reasoning`, or `stream()` for API-specific options. Both use configured providers and resolve authentication, including for providers registered with `tangent.registerProvider()`. Use these instead of `pi-ai/compat` streaming functions, which cannot see extension provider registrations.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while tangent is idle.
...
Request a graceful shutdown of tangent.
...
```typescript
tangent.on("tool_call", (event, ctx) => {
...
```typescript
tangent.on("before_agent_start", (event, ctx) => {
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `tangent` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@tangent-ai/tangent-coding-agent";

tangent.registerCommand("switch", {
...
- Captured old `tangent` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
tangent.registerCommand("handoff", {
...
```typescript
tangent.registerCommand("reload-runtime", {
...
export default function (tangent: ExtensionAPI) {
  ...
  tangent.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      tangent.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### tangent.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### tangent.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`tangent.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `tangent.getAllTools()` and are callable by the LLM without `/reload`.

Use `tangent.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@tangent-ai/tangent-ai";
...
### tangent.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`tangent.appendEntry()`](#piappendentrycustomtype-data) with [`tangent.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### tangent.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
tangent.sendUserMessage("What is 2+2?");

// With content array (text + images)
tangent.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
tangent.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
tangent.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
tangent.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### tangent.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `tangent.registerEntryRenderer()`.

```typescript
tangent.appendEntry("my-state", { count: 42 });
tangent.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
```typescript
tangent.setSessionName("Refactor auth module");
```

### tangent.getSessionName()

Get the current session name, if set.

```typescript
const name = tangent.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### tangent.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
tangent.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
tangent.setLabel(entryId, undefined);
...
### tangent.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, tangent keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
tangent.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@tangent-ai/tangent-tui";

tangent.registerCommand("deploy", {
...
```typescript
const commands = tangent.getCommands();
...
### tangent.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `tangent.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### tangent.registerMarkdownTransformer(transformer)
...
```typescript
tangent.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
...
### tangent.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `tangent.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@tangent-ai/tangent-tui";

tangent.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### tangent.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
tangent.registerShortcut("ctrl+shift+p", {
...
### tangent.registerFlag(name, options)

Register a CLI flag.

```typescript
tangent.registerFlag("plan", {
...
// Check value
if (tangent.getFlag("plan")) {
  // Plan mode enabled
}
```

### tangent.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await tangent.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### tangent.getActiveTools() / tangent.getAllTools() / tangent.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `tangent.getActiveTools()` returns the active tool names as `string[]`; `tangent.getAllTools()` returns metadata for all configured tools.

```typescript
const active = tangent.getActiveTools(); // ["read", "bash", ...]
const all = tangent.getAllTools();
// all = [{
//   name: "read",
...
tangent.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
tangent.setActiveTools(["read", "bash"]); // Switch to read-only
```

`tangent.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await tangent.setModel(model);
  if (!success) {
...
### tangent.getThinkingLevel() / tangent.setThinkingLevel(level)
...
`tangent.setThinkingLevel()` changes the thinking level for the current session. The change is recorded in session history and restored when that session is resumed, but it does not change the configured default used by new sessions.

```typescript
const current = tangent.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
tangent.setThinkingLevel("high");
```
...
```typescript
tangent.events.on("my:event", (data) => { ... });
tangent.events.emit("my:event", { ... });
```

### tangent.registerProvider(name, config)
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@tangent-ai/tangent-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@tangent-ai/tangent-ai";
...
tangent.registerProvider(provider);

// Register a new provider with custom models
tangent.registerProvider("my-proxy", {
  name: "My Proxy",
...
tangent.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
tangent.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
tangent.registerProvider("corporate-ai", {
...
### tangent.unregisterProvider(name)
...
```typescript
tangent.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    tangent.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `tangent.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `tangent.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@tangent-ai/tangent-coding-agent";
...
tangent.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via tangent.exec (captured from extension closure)
    const result = await tangent.exec("some-command", [], { signal });
...
**Important:** Use `StringEnum` from `@tangent-ai/tangent-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when tangent resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
tangent -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
tangent --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [powershell.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/powershell.ts) - `PowerShellToolDetails`
- [edit.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@tangent-ai/tangent-coding-agent";
...
For `user_bash`, extensions can reuse tangent's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
export default function (tangent: ExtensionAPI) {
  ...
  tangent.registerTool({ name: "db_connect", ... });
  tangent.registerTool({ name: "db_query", ... });
  tangent.registerTool({ name: "db_close", ... });

  tangent.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/earendil-works/tangent/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@tangent-ai/tangent-coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `tangent.setActiveTools()` during execution. Pi detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `tangent.registerTool()` so it appears in `tangent.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `tangent.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
...
You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `tangent.setActiveTools()` must already be registered; unknown names are ignored.
...
export default function (tangent: ExtensionAPI) {
  ...
  tangent.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = tangent.getAllTools()
      ...
      tangent.setActiveTools([...new Set([...active, ...added])]);
  ...
  tangent.on("session_start", () => {
    ...
    const initialTools = tangent.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    tangent.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("tangent - my-project");
...
```typescript
import { Text, Component } from "@tangent-ai/tangent-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@tangent-ai/tangent-coding-agent";
import { matchesKey } from "@tangent-ai/tangent-tui";
...
tangent.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `tangent.sendMessage()`:
...
```typescript
tangent.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

tangent.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@tangent-ai/tangent-coding-agent";
...
| `event-bus.ts` | Inter-extension events | `tangent.events` |
| **Session Metadata** |||
