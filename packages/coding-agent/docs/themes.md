> tangent can create themes. Ask it to build one for your setup.
...
- Built-in: `dark`, `light`
- Global: `~/.tangent/agent/themes/*.json`
- Project: `.pi/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `tangent.themes` entries in `package.json`
...
On first run, tangent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.tangent/agent/themes
vim ~/.tangent/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/tangent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, tangent reloads it automatically for immediate visual feedback.
...
Pi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, tangent falls back to the nearest approximation.
