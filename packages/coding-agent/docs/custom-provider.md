...
Extensions can register custom model providers via `tangent.registerProvider()`. This enables:
...
export default function (tangent: ExtensionAPI) {
  tangent.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  tangent.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. tangent waits for the factory before startup continues, so the provider is available during interactive startup and to `tangent --list-models`.
...
tangent.registerProvider("openai", {
  headers: {
...
tangent.registerProvider("google", {
...
```typescript
tangent.registerProvider("my-llm", {
...
Use `tangent.unregisterProvider(name)` to remove a provider that was previously registered via `tangent.registerProvider(name, ...)`:
...
// Later, remove it
tangent.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map tangent levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
tangent.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@tangent-ai/tangent-ai";
...
Credentials are persisted in `~/.tangent/agent/auth.json`:
...
- [anthropic-messages.ts](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/api/anthropic-messages.ts) - Anthropic Messages API
- [mistral-conversations.ts](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/api/mistral-conversations.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/api/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/api/openai-responses.ts) - OpenAI Responses API
- [google-generative-ai.ts](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/api/google-generative-ai.ts) - Google Generative AI
- [bedrock-converse-stream.ts](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/api/bedrock-converse-stream.ts) - AWS Bedrock
...
When a request exceeds the model's context window, tangent can recover automatically by compacting the conversation and retrying. This recovery only kicks in if tangent recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of tangent's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/tangent/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message tangent does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase tangent recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (tangent: ExtensionAPI) {
  ...
  tangent.on("message_end", (event, ctx) => {
...
`message_end` runs before tangent tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what tangent checks. With this in place, tangent will:
...
- Match a provider-specific pattern, not tangent's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of tangent's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/earendil-works/tangent/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps tangent thinking levels to provider/model-specific values; null marks a level unsupported. */
