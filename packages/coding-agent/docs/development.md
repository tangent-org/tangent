...
See [AGENTS.md](https://github.com/earendil-works/tangent/blob/main/AGENTS.md) for additional guidelines.

## Setup

```bash
git clone https://github.com/earendil-works/tangent
cd tangent
npm install
npm run build
```
...
`TANGENT_SERVER_DIR` overrides the server profile and socket directory (default: `~/.tangent/server`). `TANGENT_SERVER_ID` selects the logical server ID when `--server-id` is omitted.
...
`/debug` (hidden) writes to `~/.tangent/agent/pi-debug.log`:
