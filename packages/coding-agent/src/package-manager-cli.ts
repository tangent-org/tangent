...
import { Markdown, type MarkdownTheme } from "@tangent-ai/tangent-tui";
...
const DEFAULT_INSTALLER_API_BASE = "https://tangent.dev/api/installer/releases";
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|tangent] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update tangent, installed packages, or model catalogs.

Options:
  --self                  Update tangent only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update tangent and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall tangent even if the current version is latest

Short forms:
  ${APP_NAME} update                Update tangent only
  ${APP_NAME} update --all          Update tangent and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update tangent             Update tangent only (self works as alias to tangent)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "tangent";
			if (sourceIsSelf) {
