...
import * as _bundledPiAgentCore from "@tangent-ai/tangent-agent-core";
import type { Provider } from "@tangent-ai/tangent-ai";
import * as _bundledPiAiCompat from "@tangent-ai/tangent-ai/compat";
import * as _bundledPiAiOauth from "@tangent-ai/tangent-ai/oauth";
import * as _bundledPiAiProviders from "@tangent-ai/tangent-ai/providers/all";
import type { KeyId } from "@tangent-ai/tangent-tui";
import * as _bundledPiTui from "@tangent-ai/tangent-tui";
...
// avoiding a circular dependency. Extensions can import from @tangent-ai/tangent-coding-agent.
import * as _bundledTangentCodingAgent from "../../index.ts";
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@tangent-ai/tangent-agent-core": _bundledPiAgentCore,
	"@tangent-ai/tangent-tui": _bundledPiTui,
	// Extensions resolve the pi-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	"@tangent-ai/tangent-ai": _bundledPiAiCompat,
	"@tangent-ai/tangent-ai/compat": _bundledPiAiCompat,
	"@tangent-ai/tangent-ai/oauth": _bundledPiAiOauth,
	"@tangent-ai/tangent-ai/providers/all": _bundledPiAiProviders,
	"@tangent-ai/tangent-coding-agent": _bundledTangentCodingAgent,
	...
	"@mariozechner/pi-coding-agent": _bundledTangentCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	const piAgentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@tangent-ai/tangent-agent-core");
	const piTuiEntry = resolveWorkspaceOrImport("tui/dist/index.js", "@tangent-ai/tangent-tui");
	// Extensions resolve the pi-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	const piAiCompatEntry = resolveWorkspaceOrImport("ai/dist/compat.js", "@tangent-ai/tangent-ai/compat");
	const piAiOauthEntry = resolveWorkspaceOrImport("ai/dist/oauth.js", "@tangent-ai/tangent-ai/oauth");
	const piAiProvidersEntry = resolveWorkspaceOrImport(
		"ai/dist/providers/all.js",
		"@tangent-ai/tangent-ai/providers/all",
	);

	_aliases = {
		"@tangent-ai/tangent-coding-agent": piCodingAgentEntry,
		"@tangent-ai/tangent-agent-core": piAgentCoreEntry,
		"@tangent-ai/tangent-tui": piTuiEntry,
		"@tangent-ai/tangent-ai/providers/all": piAiProvidersEntry,
		"@tangent-ai/tangent-ai/compat": piAiCompatEntry,
		"@tangent-ai/tangent-ai/oauth": piAiOauthEntry,
		"@tangent-ai/tangent-ai": piAiCompatEntry,
...
export function createExtensionRuntime(): ExtensionRuntime {
	...
	const runtime: ExtensionRuntime = {
		...
		invalidate: (message) => {
			...
			state.staleMessage =
				message ??
				"This extension ctx is stale after session replacement or reload. Do not use a captured tangent or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the ctx passed to withSession. For reload, do not use the old ctx after await ctx.reload().";
 ...
 *
 * Checks for:
 * 1. package.json with "tangent.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "tangent" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "tangent" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with tangent manifest or index.ts
