...
import { type ImageContent, modelsAreEqual } from "@tangent-ai/tangent-ai";
import { setCapabilityOverrides } from "@tangent-ai/tangent-tui";
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `tangent update` drain naturally instead.
	...
	// tangent reads user-authored themes, so it opts into full validation before any theme loads.
