...
describe("regression #2860: replaced session callbacks", () => {
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale tangent/ctx", async () => {
		const events: string[] = [];
	...
	it("supports withSession for fork", async () => {
		const { runtime } = await createRuntimeForTest(
			(tangent) => {
				tangent.registerCommand("fork-it", {
					description: "fork-it",
					handler: async (_args, ctx) => {
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(tangent) => {
				tangent.registerCommand("switch-it", {
