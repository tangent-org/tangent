...
import { fauxAssistantMessage, registerFauxProvider } from "@tangent-ai/tangent-ai/compat";
...
describe("AgentSessionRuntime session lifecycle events", () => {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtimeHost } = await createRuntimeHost((tangent) => {
			tangent.on("session_before_switch", (event) => {
				events.push(event);
			});
			tangent.on("session_shutdown", (event) => {
				events.push(event);
			});
			tangent.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtimeHost } = await createRuntimeHost((tangent) => {
			tangent.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
