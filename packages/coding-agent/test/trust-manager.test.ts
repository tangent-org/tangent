...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".tangent", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".tangent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".tangent", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".tangent"), { recursive: true });
			writeFileSync(join(cwd, ".tangent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".tangent"), { recursive: true, force: true });
