...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	...
	try {
		await writeManifest(root, "packages/ai", {
			name: "@tangent-ai/tangent-ai",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/coding-agent", {
			name: "@tangent-ai/tangent-coding-agent",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			name: "@tangent-ai/tangent-evals",
			version: "9.9.9",
			private: true,
			dependencies: {
				"@tangent-ai/tangent-coding-agent": "^1.0.0",
				"@mariozechner/pi-ai": "npm:@tangent-ai/tangent-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@tangent-ai/tangent-coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@mariozechner/pi-ai"], "npm:@tangent-ai/tangent-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@tangent-ai/tangent-coding-agent"], "^1.0.0");
